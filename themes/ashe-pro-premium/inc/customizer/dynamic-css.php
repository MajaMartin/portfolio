...
*/
	...
	$css .= '
		...
		.sidebar-alt-close-btn span {
			background-color: '. ashe_options( 'colors_content_title' ) .';
		}
...
*/
	...
	// Logo
	if ( ashe_options( 'responsive_logo' ) !== '' ) {
		$css .= '
	...
	// Mini Logo
	if ( ashe_options( 'responsive_mini_logo' ) === false ) {
		$css .= '
		@media screen and ( max-width: 768px ) {		
			.mini-logo {
				display: none;	
	...
	@media screen and ( max-width: 768px ) {		
		.mini-logo a {
			max-width: '. ashe_options( 'responsive_mini_logo_width' ) .'px !important;
		}	 
	}
	';
	...
	// Fixed Navigation
	if ( ashe_options( 'responsive_nav_fixed' ) !== true && is_customize_preview() ) {
		$css .= '
		#page-header {
...
*/
	...
	if ( ashe_options( 'main_nav_label' ) === true || ! has_nav_menu('top') || ashe_options( 'top_bar_show_menu' ) === false ) {
		$css .= "
...
*/
	...
	// Video / Parallax
	if ( ashe_options( 'header_image_bg_type' ) === 'video' || ashe_options( 'header_image_parallax' ) === true ) {
		$css .= '
			.entry-header {
				background-color: transparent !important;
			}
...
*/
	...
	$css .= '
		...
		.mini-logo + .main-nav-socials-trigger {
			  right: '. ( ashe_options( 'general_content_padding' ) + 30 ) .'px;
			  left: auto;
		}
	...
	if ( (int)ashe_options( 'featured_slider_columns' ) === 1 ) {
		...
		$css .= '
			#featured-slider.boxed-wrapper {
				max-width: '. ashe_options( 'featured_slider_crop_width' ) .'px;
			}
			.slider-item-bg {
			  height: '. ashe_options( 'featured_slider_crop_height' ) .'px;
			}
		';
	} else {
		// Horizontal Gutter
		$css .= '
			#featured-slider .slick-slide {
				margin-left: '. ashe_options( 'featured_slider_gutter_horz' ) .'px;
			}
...
*/
	...
	$featured_links =  (int)ashe_options( 'featured_links_columns' );
	...
	if ( ashe_options( 'featured_links_gutter_horz' ) === true ) {
		...
		$css .= '
			#featured-links .featured-link {
				margin-top: '. $featured_links_gutter .'px;
			}
			...
			#featured-links .featured-link:nth-of-type('.$featured_links.'n) {
				margin-right: 0;
			}
		';
	}
	...
	if ( ashe_options( 'featured_links_title_6' ) === '' ) {
		$css .= '
...
*/
	...
	if ( ! is_rtl() ) {
		...
		$css .= '
			...
			[data-layout*="col3"] .blog-grid > li:nth-of-type(3n+1),
			[data-layout*="col4"] .blog-grid > li:nth-of-type(4n+1) {
				margin-right: 0;
			}
		';
	}
	...
	if ( ashe_options( 'single_page_show_dropcups' ) === true ) {
		$css .= "
			...
			.single .post-content .elementor-text-editor p:first-of-type:first-letter {
			  	float: left;
				margin: 0px 12px 0 0;
				font-family: '". str_replace( '+', ' ', ashe_options( 'typography_head_family' ) ) ."';	
				font-size: 80px;
...
*/
	...
	if ( ashe_options('page_footer_copyright') === '' ) {
		$css .= '
			.footer-menu-container:after {
				display: none;
			}
